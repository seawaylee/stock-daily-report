...
def generate_prompt(industry_inflow, industry_outflow, output_path="results/sector_flow_image_prompt.txt"):
    ...
    for _, row in top10_out.iterrows():
        val = float(row['net_flow_billion'])
        c_name = clean_sector_name(row['名称'])
        worshipper_labels.append(f"{c_name} ({val:.1f}亿)")

    # Prepare summary for content details
    top5_in = industry_inflow.head(5)
    top5_out = industry_outflow.head(5)

    inflow_summary = ", ".join([f"{i+1}. {clean_sector_name(row['名称'])} (+{row['net_flow_billion']:.1f}亿)"
                                 for i, (_, row) in enumerate(top5_in.iterrows())])
    outflow_summary = ", ".join([f"{i+1}. {clean_sector_name(row['名称'])} ({row['net_flow_billion']:.1f}亿)"
...
1. **THE GIANTS (Top Inflow Sectors)**:
   ...
   - **CENTER**: Object for **"{giant_entries[0][0]}"**. Label: "**{giant_entries[0][0]}**" (Black Bold Ink) & "+{giant_entries[0][1]:.1f}亿" (Red).
   - **LEFT**: Object for **"{giant_entries[1][0]}"**. Label: "**{giant_entries[1][0]}**" (Black Bold) & "+{giant_entries[1][1]:.1f}亿".
   - **RIGHT**: Object for **"{giant_entries[2][0]}"**. Label: "**{giant_entries[2][0]}**" (Black Bold) & "+{giant_entries[2][1]:.1f}亿".
...
(Optimized for Nano Banana Pro: Vintage Art Style + Clear Text enforcement)"""
    ...
    print(f"✅ Image Prompt saved to {output_path} (Format: Midjourney/SD)")

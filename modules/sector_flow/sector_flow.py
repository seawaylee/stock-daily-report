...
def draw_sector_chart(inflow_df, outflow_df, name_col, flow_col, output_dir):
    ...
    fig.text(0.5, 0.95, f"板块资金流向大比拼 ({date_str})",
    ...
    ax_in.set_title("主力净流入 Top 10 (亿元)", color='white', fontsize=16, pad=10)
    ...
    ax_out.set_title("主力净流出 Top 10 (亿元)", color='white', fontsize=16, pad=10)
...
def generate_prompt(industry_inflow, industry_outflow, output_path="results/sector_flow_image_prompt.txt"):
    """
    Generate AI Prompt in "Vintage Paper/Hand-drawn Infographic" style (Matching Market Ladder)
    """
    # Helper: Clean Name
    import re
    ...
    # Prepare Data Text
    top_in_lines = []
    for i, (_, row) in enumerate(industry_inflow.head(5).iterrows()):
         val = float(row['net_flow_billion'])
         c_name = clean_sector_name(row['名称'])
         top_in_lines.append(f"{i+1}. {c_name} (+{val:.1f}亿)")

    top_out_lines = []
    for i, (_, row) in enumerate(industry_outflow.head(5).iterrows()):
         val = float(row['net_flow_billion'])
         c_name = clean_sector_name(row['名称'])
         top_out_lines.append(f"{i+1}. {c_name} (-{abs(val):.1f}亿)")

    inflow_text = ", ".join(top_in_lines)
    outflow_text = ", ".join(top_out_lines)

    # Creative Title
    top_names_for_title = list(industry_inflow.head(3)['名称'])
    selected_title = get_creative_title(top_names_for_title)

    prompt_content = f"""
Hand-drawn infographic poster, Chinese A-share Sector Fund Flow (资金流向), {date_str}.

**Style**: Warm cream paper texture (#F5E6C8), vintage notebook aesthetic, handwritten Chinese fonts.
**Visual Elements**:
- Diverging Bar Chart (Red bars for Inflow, Green bars for Outflow).
- List of top sectors with financial numbers.
- Paper texture background with ink annotations.

**Layout**: 9:16 vertical.
**Title**: "{selected_title} - 资金流向".

**Content Details**:
- **Top Inflow (Red)**: {inflow_text}
- **Top Outflow (Green)**: {outflow_text}

**Atmosphere**: Professional, detailed financial analysis, retro financial newspaper style.
...
"""
    ...
    print(f"Image Prompt saved to {output_path} (Style: Vintage Paper)")

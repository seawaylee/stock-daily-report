...
3. **生成文案**：再次运行命令，程序检测到分析结果，生成小红书任务 `task_xiaohongshu.txt` 后暂停。
4. **AI 写稿**：Agent 读取任务，将合规文案写入 `agent_outputs/result_xiaohongshu.txt`。
5. **最终产出**：第三次运行命令，程序生成最终的 **图片提示词**。
...
>
> Agent 会代替你执行“运行脚本 -> 读取任务 -> AI写作 -> 再次运行”的循环，直到生成最终结果。
